...
public:
  ReaderNewInstanceWorker(const Napi::Promise::Deferred &deferred,
                          pulsar_client_t *cClient, ReaderConfig *readerConfig)
  ...
  void Execute() {
    ...
    if (topic.empty()) {
      SetError(std::string("Topic is required and must be specified as a "
                           "string when creating reader"));
      return;
    }
    if (this->readerConfig->GetCStartMessageId() == nullptr) {
      SetError(std::string("StartMessageId is required and must be specified "
                           "as a MessageId object when creating reader"));
      return;
    }

    pulsar_result result = pulsar_client_create_reader(
        this->cClient, topic.c_str(), this->readerConfig->GetCStartMessageId(),
...
Napi::Value Reader::NewInstance(const Napi::CallbackInfo &info,
                                pulsar_client_t *cClient) {
  Napi::Promise::Deferred deferred = Napi::Promise::Deferred::New(info.Env());
  Napi::Object config = info[0].As<Napi::Object>();
  ReaderConfig *readerConfig = new ReaderConfig(config);
  ReaderNewInstanceWorker *wk =
      new ReaderNewInstanceWorker(deferred, cClient, readerConfig);
  wk->Queue();
...
public:
  ReaderReadNextWorker(const Napi::Promise::Deferred &deferred,
                       pulsar_reader_t *cReader, int64_t timeout = -1)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cReader(cReader), timeout(timeout) {}
...
Napi::Value Reader::ReadNext(const Napi::CallbackInfo &info) {
  ...
  if (info[0].IsUndefined()) {
    ReaderReadNextWorker *wk =
        new ReaderReadNextWorker(deferred, this->cReader);
    wk->Queue();
  } else {
...
Napi::Value Reader::HasNext(const Napi::CallbackInfo &info) {
  int value = 0;
  pulsar_result result =
      pulsar_reader_has_message_available(this->cReader, &value);
  if (result != pulsar_result_Ok) {
    Napi::Error::New(info.Env(), "Failed to check if next message is available")
...
public:
  ReaderCloseWorker(const Napi::Promise::Deferred &deferred,
                    pulsar_reader_t *cReader)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cReader(cReader) {}
  ~ReaderCloseWorker() {}
  void Execute() {
  ...
  void OnError(const Napi::Error &e) {
    this->deferred.Reject(
        Napi::Error::New(Env(),
                         std::string("Failed to close reader: ") + e.Message())
            .Value());
  }
