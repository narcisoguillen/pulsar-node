...
Client::Client(const Napi::CallbackInfo &info)
  ...
  if (!clientConfig.Has(CFG_SERVICE_URL) ||
    ...
    if (clientConfig.Get(CFG_SERVICE_URL).ToString().Utf8Value().empty()) {
      Napi::Error::New(
          env, "Service URL is required and must be specified as a string")
  ...
  if (clientConfig.Has(CFG_AUTH) && clientConfig.Get(CFG_AUTH).IsObject()) {
    ...
    if (obj.Has(CFG_AUTH_PROP) && obj.Get(CFG_AUTH_PROP).IsObject()) {
      ...
      pulsar_client_configuration_set_auth(cClientConfig,
  ...
  if (clientConfig.Has(CFG_OP_TIMEOUT) &&
    ...
    if (operationTimeoutSeconds > 0) {
      pulsar_client_configuration_set_operation_timeout_seconds(
          cClientConfig, operationTimeoutSeconds);
    }
  }
  ...
  if (clientConfig.Has(CFG_LISTENER_THREADS) &&
      clientConfig.Get(CFG_LISTENER_THREADS).IsNumber()) {
    int32_t messageListenerThreads =
        clientConfig.Get(CFG_LISTENER_THREADS).ToNumber().Int32Value();
    if (messageListenerThreads > 0) {
      pulsar_client_configuration_set_message_listener_threads(
          cClientConfig, messageListenerThreads);
    }
  }

  if (clientConfig.Has(CFG_CONCURRENT_LOOKUP) &&
      clientConfig.Get(CFG_CONCURRENT_LOOKUP).IsNumber()) {
    int32_t concurrentLookupRequest =
        clientConfig.Get(CFG_CONCURRENT_LOOKUP).ToNumber().Int32Value();
    if (concurrentLookupRequest > 0) {
      pulsar_client_configuration_set_concurrent_lookup_request(
          cClientConfig, concurrentLookupRequest);
    }
  }
  ...
  if (clientConfig.Has(CFG_TLS_TRUST_CERT) &&
    ...
    pulsar_client_configuration_set_tls_trust_certs_file_path(
        cClientConfig, tlsTrustCertsFilePath.Utf8Value().c_str());
  }
  ...
  if (clientConfig.Has(CFG_TLS_VALIDATE_HOSTNAME) &&
    ...
    pulsar_client_configuration_set_validate_hostname(
        cClientConfig, tlsValidateHostname.Value());
  }

  if (clientConfig.Has(CFG_TLS_ALLOW_INSECURE) &&
      clientConfig.Get(CFG_TLS_ALLOW_INSECURE).IsBoolean()) {
    Napi::Boolean tlsAllowInsecureConnection =
        clientConfig.Get(CFG_TLS_ALLOW_INSECURE).ToBoolean();
    pulsar_client_configuration_set_tls_allow_insecure_connection(
        cClientConfig, tlsAllowInsecureConnection.Value());
  }

  if (clientConfig.Has(CFG_STATS_INTERVAL) &&
      clientConfig.Get(CFG_STATS_INTERVAL).IsNumber()) {
    uint32_t statsIntervalInSeconds =
        clientConfig.Get(CFG_STATS_INTERVAL).ToNumber().Uint32Value();
    if (statsIntervalInSeconds > 0) {
      pulsar_client_configuration_set_stats_interval_in_seconds(
          cClientConfig, statsIntervalInSeconds);
    }
  }
...
public:
  ClientCloseWorker(const Napi::Promise::Deferred &deferred,
                    pulsar_client_t *cClient)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cClient(cClient) {}
  ~ClientCloseWorker() {}
  void Execute() {
  ...
  void OnError(const Napi::Error &e) {
    this->deferred.Reject(
        Napi::Error::New(Env(),
                         std::string("Failed to close client: ") + e.Message())
            .Value());
  }
