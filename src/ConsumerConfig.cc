...
ConsumerConfig::ConsumerConfig(const Napi::Object &consumerConfig)
  ...
  if (consumerConfig.Has(CFG_SUBSCRIPTION) &&
      consumerConfig.Get(CFG_SUBSCRIPTION).IsString()) {
    this->subscription =
        consumerConfig.Get(CFG_SUBSCRIPTION).ToString().Utf8Value();
  }

  if (consumerConfig.Has(CFG_SUBSCRIPTION_TYPE) &&
      consumerConfig.Get(CFG_SUBSCRIPTION_TYPE).IsString()) {
    std::string subscriptionType =
        consumerConfig.Get(CFG_SUBSCRIPTION_TYPE).ToString().Utf8Value();
    if (SUBSCRIPTION_TYPE.count(subscriptionType)) {
      pulsar_consumer_configuration_set_consumer_type(
          this->cConsumerConfig, SUBSCRIPTION_TYPE.at(subscriptionType));
    }
  }
  ...
  if (consumerConfig.Has(CFG_CONSUMER_NAME) &&
    ...
    if (!consumerName.empty())
      pulsar_consumer_set_consumer_name(this->cConsumerConfig,
                                        consumerName.c_str());
  }

  if (consumerConfig.Has(CFG_ACK_TIMEOUT) &&
      consumerConfig.Get(CFG_ACK_TIMEOUT).IsNumber()) {
    this->ackTimeoutMs =
        consumerConfig.Get(CFG_ACK_TIMEOUT).ToNumber().Int64Value();
    if (this->ackTimeoutMs == 0 ||
        this->ackTimeoutMs >= MIN_ACK_TIMEOUT_MILLIS) {
      pulsar_consumer_set_unacked_messages_timeout_ms(this->cConsumerConfig,
                                                      this->ackTimeoutMs);
    }
  }
  ...
  if (consumerConfig.Has(CFG_RECV_QUEUE) &&
    ...
    if (receiverQueueSize >= 0) {
      pulsar_consumer_configuration_set_receiver_queue_size(
          this->cConsumerConfig, receiverQueueSize);
    }
  }
  ...
  if (consumerConfig.Has(CFG_RECV_QUEUE_ACROSS_PARTITIONS) &&
    ...
    int32_t receiverQueueSizeAcrossPartitions =
        consumerConfig.Get(CFG_RECV_QUEUE_ACROSS_PARTITIONS)
            .ToNumber()
            .Int32Value();
  ...
  if (consumerConfig.Has(CFG_PROPS) &&
    ...
    for (int i = 0; i < size; i++) {
      ...
      pulsar_consumer_configuration_set_property(this->cConsumerConfig,
...
pulsar_consumer_configuration_t *ConsumerConfig::GetCConsumerConfig() {
