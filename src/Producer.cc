...
void Producer::Init(Napi::Env env, Napi::Object exports) {
  ...
  Napi::Function func =
      DefineClass(env, "Producer",
                  {InstanceMethod("send", &Producer::Send),
                   InstanceMethod("flush", &Producer::Flush),
...
void Producer::SetCProducer(pulsar_producer_t *cProducer) {
  ...
  void Execute() {
    ...
    if (topic.empty()) {
      SetError(std::string("Topic is required and must be specified as a "
                           "string when creating producer"));
      return;
    }

    pulsar_result result = pulsar_client_create_producer(
        this->cClient, topic.c_str(),
        this->producerConfig->GetCProducerConfig(), &(this->cProducer));
    delete this->producerConfig;
    if (result != pulsar_result_Ok) {
      SetError(std::string("Failed to create producer: ") +
...
Napi::Value Producer::NewInstance(const Napi::CallbackInfo &info,
  ...
  ProducerNewInstanceWorker *wk =
      new ProducerNewInstanceWorker(deferred, cClient, producerConfig);
  wk->Queue();
  return deferred.Promise();
}

Producer::Producer(const Napi::CallbackInfo &info)
    : Napi::ObjectWrap<Producer>(info) {}

class ProducerSendWorker : public Napi::AsyncWorker {
public:
  ProducerSendWorker(const Napi::Promise::Deferred &deferred,
                     pulsar_producer_t *cProducer, pulsar_message_t *cMessage)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cProducer(cProducer), cMessage(cMessage) {}
  ~ProducerSendWorker() { pulsar_message_free(this->cMessage); }
  void Execute() {
    pulsar_result result =
        pulsar_producer_send(this->cProducer, this->cMessage);
    if (result != pulsar_result_Ok)
  ...
  void OnError(const Napi::Error &e) {
    this->deferred.Reject(
        Napi::Error::New(Env(),
                         std::string("Failed to send message: ") + e.Message())
            .Value());
  }
...
Napi::Value Producer::Send(const Napi::CallbackInfo &info) {
  Napi::Promise::Deferred deferred = Napi::Promise::Deferred::New(info.Env());
  pulsar_message_t *cMessage =
      Message::BuildMessage(info[0].As<Napi::Object>());
  ProducerSendWorker *wk =
      new ProducerSendWorker(deferred, this->cProducer, cMessage);
  wk->Queue();
  return deferred.Promise();
}
...
public:
  ProducerFlushWorker(const Napi::Promise::Deferred &deferred,
                      pulsar_producer_t *cProducer)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cProducer(cProducer) {}
  ...
  void OnError(const Napi::Error &e) {
    this->deferred.Reject(
        Napi::Error::New(Env(), std::string("Failed to flush producer: ") +
                                    e.Message())
            .Value());
  }
...
public:
  ProducerCloseWorker(const Napi::Promise::Deferred &deferred,
                      pulsar_producer_t *cProducer)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cProducer(cProducer) {}
  ...
  void OnError(const Napi::Error &e) {
    this->deferred.Reject(
        Napi::Error::New(Env(), std::string("Failed to close producer: ") +
                                    e.Message())
            .Value());
  }
