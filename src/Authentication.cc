...
Authentication::Authentication(const Napi::CallbackInfo &info)
  ...
  if (info.Length() < 1 || !info[0].IsString() ||
      info[0].ToString().Utf8Value().empty()) {
    Napi::Error::New(env, "Authentication method is not specified")
  ...
  if (authMethod == "tls" || authMethod == "token") {
    if (info.Length() < 2 || !info[1].IsObject()) {
      Napi::Error::New(env, "Authentication parameter must be a object")
          .ThrowAsJavaScriptException();
      return;
    }
    ...
    if (authMethod == "tls") {
      if (!obj.Has(PARAM_TLS_CERT) || !obj.Get(PARAM_TLS_CERT).IsString() ||
          !obj.Has(PARAM_TLS_KEY) || !obj.Get(PARAM_TLS_KEY).IsString()) {
        Napi::Error::New(env, "Missing required parameter")
            .ThrowAsJavaScriptException();
        return;
      }
      this->cAuthentication = pulsar_authentication_tls_create(
          obj.Get(PARAM_TLS_CERT).ToString().Utf8Value().c_str(),
      ...
      this->cAuthentication = pulsar_authentication_token_create(
          obj.Get(PARAM_TOKEN).ToString().Utf8Value().c_str());
    }
    ...
    this->cAuthentication = pulsar_authentication_athenz_create(
        info[1].ToString().Utf8Value().c_str());
  } else {
    Napi::Error::New(env, "Unsupported authentication method")
        .ThrowAsJavaScriptException();
    return;
  }
}
...
pulsar_authentication_t *Authentication::GetCAuthentication() {
