...
Message::Message(const Napi::CallbackInfo &info)
    : Napi::ObjectWrap<Message>(info), cMessage(nullptr) {}
...
Napi::Value Message::GetProperties(const Napi::CallbackInfo &info) {
  ...
  pulsar_string_map_t *cProperties =
      pulsar_message_get_properties(this->cMessage);
  int size = pulsar_string_map_size(cProperties);
  for (int i = 0; i < size; i++) {
    arr.Set(pulsar_string_map_get_key(cProperties, i),
            pulsar_string_map_get_value(cProperties, i));
  }
  return arr;
}
...
Napi::Value Message::GetEventTimestamp(const Napi::CallbackInfo &info) {
  ...
  return Napi::Number::New(env,
                           pulsar_message_get_event_timestamp(this->cMessage));
}
...
Napi::Value Message::GetPublishTimestamp(const Napi::CallbackInfo &info) {
  ...
  return Napi::Number::New(
      env, pulsar_message_get_publish_timestamp(this->cMessage));
}
...
Napi::Value Message::GetPartitionKey(const Napi::CallbackInfo &info) {
  ...
  return Napi::String::New(env,
                           pulsar_message_get_partitionKey(this->cMessage));
}
...
bool Message::ValidateCMessage(Napi::Env env) {
  if (this->cMessage) {
    ...
    Napi::Error::New(env, "Message has not been built")
...
pulsar_message_t *Message::BuildMessage(Napi::Object conf) {
  ...
  if (conf.Has(CFG_PROPS) && conf.Get(CFG_PROPS).IsObject()) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      pulsar_message_set_property(cMessage, key.Utf8Value().c_str(),
  ...
  if (conf.Has(CFG_PARTITION_KEY) && conf.Get(CFG_PARTITION_KEY).IsString()) {
    ...
    pulsar_message_set_partition_key(cMessage,
                                     partitionKey.Utf8Value().c_str());
  }
  ...
  if (conf.Has(CFG_REPL_CLUSTERS) && conf.Get(CFG_REPL_CLUSTERS).IsArray()) {
    ...
    if (length == 0) {
      ...
      // TODO: temoporalily commented out unless 2.3.1 which includes interface
      // change of pulsar_message_set_replication_clusters (#3729) is released
