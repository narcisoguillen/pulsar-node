...
void Consumer::SetCConsumer(pulsar_consumer_t *cConsumer) {
  this->cConsumer = cConsumer;
}

Consumer::Consumer(const Napi::CallbackInfo &info)
    : Napi::ObjectWrap<Consumer>(info) {}

class ConsumerNewInstanceWorker : public Napi::AsyncWorker {
public:
  ConsumerNewInstanceWorker(const Napi::Promise::Deferred &deferred,
  ...
  void Execute() {
    ...
    if (topic.empty()) {
      SetError(std::string("Topic is required and must be specified as a "
                           "string when creating consumer"));
      return;
    }
    const std::string &subscription = this->consumerConfig->GetSubscription();
    if (subscription.empty()) {
      SetError(std::string("Subscription is required and must be specified as "
    ...
    pulsar_result result = pulsar_client_subscribe(
        this->cClient, topic.c_str(), subscription.c_str(),
        this->consumerConfig->GetCConsumerConfig(), &(this->cConsumer));
    delete this->consumerConfig;
    if (result != pulsar_result_Ok) {
      SetError(std::string("Failed to create consumer: ") +
...
public:
  ConsumerReceiveWorker(const Napi::Promise::Deferred &deferred,
                        pulsar_consumer_t *cConsumer, int64_t timeout = -1)
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cConsumer(cConsumer), timeout(timeout) {}
  ~ConsumerReceiveWorker() {}
  void Execute() {
    pulsar_result result;
    if (timeout > 0) {
      result = pulsar_consumer_receive_with_timeout(this->cConsumer,
                                                    &(this->cMessage), timeout);
    } else {
...
Napi::Value Consumer::Receive(const Napi::CallbackInfo &info) {
  ...
  if (info[0].IsUndefined()) {
    ConsumerReceiveWorker *wk =
        new ConsumerReceiveWorker(deferred, this->cConsumer);
    wk->Queue();
  } else {
    Napi::Number timeout = info[0].As<Napi::Object>().ToNumber();
    ConsumerReceiveWorker *wk = new ConsumerReceiveWorker(
        deferred, this->cConsumer, timeout.Int64Value());
    wk->Queue();
  }
  return deferred.Promise();
}
...
void Consumer::Acknowledge(const Napi::CallbackInfo &info) {
  ...
  pulsar_consumer_acknowledge_async(this->cConsumer, msg->GetCMessage(), NULL,
                                    NULL);
}
...
void Consumer::AcknowledgeId(const Napi::CallbackInfo &info) {
  ...
  pulsar_consumer_acknowledge_async_id(this->cConsumer, msgId->GetCMessageId(),
                                       NULL, NULL);
}
...
void Consumer::AcknowledgeCumulative(const Napi::CallbackInfo &info) {
  Napi::Object obj = info[0].As<Napi::Object>();
  Message *msg = Message::Unwrap(obj);
  pulsar_consumer_acknowledge_cumulative_async(this->cConsumer,
...
void Consumer::AcknowledgeCumulativeId(const Napi::CallbackInfo &info) {
  ...
  pulsar_consumer_acknowledge_cumulative_async_id(
      this->cConsumer, msgId->GetCMessageId(), NULL, NULL);
}
...
public:
  ConsumerCloseWorker(const Napi::Promise::Deferred &deferred,
      ...
      : AsyncWorker(Napi::Function::New(deferred.Promise().Env(),
                                        [](const Napi::CallbackInfo &info) {})),
        deferred(deferred), cConsumer(cConsumer) {}
  ~ConsumerCloseWorker() {}
  void Execute() {
